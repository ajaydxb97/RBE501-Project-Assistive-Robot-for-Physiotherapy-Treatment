...
a = 0.31
theta = q_result[3]+math.pi

q_result_new = np.copy(q_result)
coef = 1
while theta<(165*math.pi/180):
        new_path_start = np.array([0,1,0,0.25,0,0,1,0.25,1,0,0,-0.03,0,0,0,1]).reshape((4,4)) #[0,0,-1,0.25,-1,0,0,-0.06,0,1,0,0.34,0,0,0,1]
        ...
        desiredPosition[0:3,0:3] = np.array([math.sin(theta),math.cos(theta),0,0,0,1,math.cos(theta),-math.sin(theta),0]).reshape((3,3))
        plt.figure(1)

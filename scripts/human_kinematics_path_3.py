...
a = math.sqrt(0.34**2 + 0.31**2)
beta = math.atan2(0.34,0.31)
theta = beta

q_result_new = np.copy(q_result)
coef = 1
while theta<(beta+120*math.pi/180):
        new_path_start = np.array([0,0,1,0.25,-math.sin(beta),math.cos(beta),0,0.25,math.cos(beta),math.sin(beta),0,a,0,0,0,1]).reshape((4,4)) #[0,0,-1,0.25,-1,0,0,-0.06,0,1,0,0.34,0,0,0,1]
        desiredPosition = np.copy(new_path_start)
        desiredPosition[0,3] = new_path_start[0,3] + (a*math.sin(theta))
        desiredPosition[1,3] = new_path_start[1,3]
        desiredPosition[2,3] = new_path_start[2,3] - (a*(1-math.cos(theta)))
        desiredPosition[0:3,0:3] = np.array([-math.sin(beta-theta),math.cos(beta-theta),0,0,0,1,math.cos(beta-theta),math.sin(beta-theta),0]).reshape((3,3))
        plt.figure(1)
        plt.title('Reference Path')
        plt.scatter(desiredPosition[0,3],desiredPosition[2,3])
        plt.xlabel('x')
        plt.ylabel('z')
        ...
        plt.figure(2)
        plt.scatter(fwd[0,3],fwd[2,3],color='green')
        ...
        time.sleep(0.01)
        plt.figure(1)
        plt.scatter(tip.get_pose()[0],tip.get_pose()[2],color='red')

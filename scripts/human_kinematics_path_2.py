...
a = 0.34
theta = q_result[0]+math.pi
q_result_new = np.copy(q_result)
coef = 1
while theta>(90.0*math.pi/180):
        new_path_start = np.array([1,0,0,0.56,0,0,-1,0.25,0,1,0,0.34,0,0,0,1]).reshape((4,4))
        desiredPosition = np.copy(new_path_start)
        desiredPosition[0,3] = new_path_start[0,3]
        desiredPosition[1,3] = new_path_start[1,3] + (a*math.sin(theta))
        desiredPosition[2,3] = new_path_start[2,3] - (a*(1-math.cos(theta)))
        desiredPosition[0:3,0:3] = np.array([1,0,0,0,math.sin(theta),math.cos(theta),0,-math.cos(theta),math.sin(theta)]).reshape((3,3))
        plt.figure(1)
        plt.title('Reference Path')
        plt.scatter(desiredPosition[1,3],desiredPosition[2,3])
        plt.xlabel('y')
        plt.ylabel('z')
        ...
        plt.figure(2)
        plt.scatter(fwd[1,3],fwd[2,3],color='green')
        ...
        time.sleep(0.01)
        plt.figure(1)
        plt.scatter(tip.get_pose()[1],tip.get_pose()[2],color='red')
